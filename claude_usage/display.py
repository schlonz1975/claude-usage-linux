...
class StatusIconRenderer:
    def __init__(self, directory: Path | None = None) -> None:
        ...
        self.directory = directory or cache_home / "claude-usage" / "icons"
    ...
    def render(self, five_hour_remaining: float | None, weekly_remaining: float | None) -> Path:
        ...
        if not path.exists():
            rings = []
            for radius, remaining, color in zip((28, 21.5), values, ("#D97757", "#F0B48A")):
                rings.append(f'<circle cx="32" cy="32" r="{radius}" stroke="#3a2e22"/>')
                if remaining >= 100:
