...
def parse_limits(result: dict[str, Any]) -> list[LimitSnapshot]:
    ...
    if isinstance(limits_by_id, dict):
        ...
        snapshots.extend(_parse_limit("claude", result["rateLimits"]))

    return sorted(
        snapshots,
        key=lambda snapshot: (
            snapshot.limit_id != "claude",
            snapshot.limit_id,
...
def display_snapshots(snapshots: list[LimitSnapshot]) -> list[LimitSnapshot]:
    claude = [snapshot for snapshot in snapshots if snapshot.limit_id == "claude"]
    if claude:
        return claude[:2]
    if not snapshots:
        return []
...
def _parse_limit(limit_id: str, value: dict[str, Any]) -> list[LimitSnapshot]:
    ...
    name = raw_name if isinstance(raw_name, str) and raw_name else (
        "Claude" if limit_id == "claude" else limit_id
    )
